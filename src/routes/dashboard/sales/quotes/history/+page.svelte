<script lang="ts">
    ...
    function canDeleteQuote(quote: any) {
        const st = String(quote?.status ?? '').trim();
        return !quote?.anulado && st === '0';
    }

    function canEditQuote(quote: any) {
        const st = String(quote?.status ?? '').trim();
        return !quote?.anulado && st === '0';
    }
...
<div class="space-y-6" in:fade>
    ...
    <!-- SEARCH & FILTERS ROW -->
    <div class="glass p-4 rounded-3xl border border-border-subtle shadow-2xl grid grid-cols-1 md:grid-cols-4 gap-4 items-center relative z-10 mb-8 w-full">
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        >
            <div class="p-8 text-center space-y-6">
                    ...
                    use:enhance={() => {
                        ...
                        return async ({ result, update }) => {
                            ...
                            if (result.type === 'success') {
                                ...
                                toast.success((result as any).data?.message || 'Cotización eliminada con éxito');
                ...
                >
                    <input type="hidden" name="doc_num" value={quoteToDelete?.doc_num} />
