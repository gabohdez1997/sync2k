                    ...
                    <div class="footer-block mt-auto">
                        <div class="footer-grid">
                            ...
                            <div class="totals-box">
                                {#if isUSD}
                                    <div class="total-row"><span>Subtotal ($)</span><span>{formatCurrency(Number(quote.total_bruto) / Number(quote.tasa || 1))}</span></div>
                                    <div class="total-row"><span>IVA (16%)</span><span>{formatCurrency(Number(quote.monto_imp) / Number(quote.tasa || 1))}</span></div>
                                    <div class="grand-total-outline">
                                        <div class="bs-total"><span class="label">Total General</span><span class="val">$ {formatCurrency(Number(quote.total_neto) / Number(quote.tasa || 1))}</span></div>
                                    </div>
                                {:else}
                                    <div class="total-row"><span>Subtotal (Bs.)</span><span>{formatCurrency(quote.total_bruto)}</span></div>
                                    <div class="total-row"><span>IVA (16%)</span><span>{formatCurrency(quote.monto_imp)}</span></div>
                                    <div class="grand-total-outline">
                                        <div class="bs-total"><span class="label">Total General</span><span class="val">Bs. {formatCurrency(quote.total_neto)}</span></div>
                                        <div class="usd-reference"><span>Referencia</span><strong>$ {formatCurrency(Number(quote.total_neto) / Number(quote.tasa_actual && Number(quote.tasa) === 1 ? quote.tasa_actual : quote.tasa || 1))}</strong></div>
                                    </div>
                                {/if}
                            </div>
                        </div>
