...
export const load: PageServerLoad = protectLoad('pur_lines', async ({ locals, fetch }) => {
    try {
        ...
        if (!dbBranches || dbBranches.length === 0) {
            return { lines: [], error: 'No hay sucursales configuradas.' };
        }

        const firstBranch = dbBranches.find(b => b.agent_url);
        if (!firstBranch) return { lines: [], error: 'No hay agentes disponibles.' };
        ...
        const res = await agent.request<any>('/catalogos/lineas').catch(() => ({ data: [] }));
        const lines = (res as any).data || [];

        return {
            lines,
            crud: userProfile?.permissions?.pur_lines || { create: false, update: false, delete: false, read: true }
        };
    } catch (e: any) {
        return { lines: [], error: `Error: ${e.message}` };
    }
});

export const actions: Actions = {
    saveLine: protectAction('pur_lines', async ({ request, locals, fetch }) => {
        const formData = await request.formData();
        const co_lin = formData.get('co_lin')?.toString().trim();
        const lin_des = formData.get('lin_des')?.toString().trim();
        ...
        for (const branch of dbBranches) {
            ...
            try {
                const endpoint = is_new ? '/catalogos/lineas' : `/catalogos/lineas/${encodeURIComponent(co_lin)}`;
                const method = is_new ? 'POST' : 'PUT';
                const res = await agent.request(endpoint, {
                    method,
                    body: JSON.stringify({ co_lin, lin_des })
                });
