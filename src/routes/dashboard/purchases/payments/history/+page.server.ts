// src/routes/dashboard/purchases/payments/history/+page.server.ts
...
export const load: PageServerLoad = protectLoad('pur_payments', async ({ url, locals, fetch }) => {
			...
			if (res.ok) {
				...
				if (resData.success) {
					...
					payments = payments.map((p: any) => {
						...
						return {
							...p,
							user_name: cashiersMap[code] || p.co_us_in
						};
					});
				} else {
					errorMsg = resData.message || 'Error al obtener pagos del agente.';
				}
			} else {
	...
	return {
		...
		filters: {
			...
			co_us_in,
			fec_d: url.searchParams.get('fec_d') || '',
			fec_h: url.searchParams.get('fec_h') || ''
		},
		error: errorMsg || null
	};
});
...
export const actions = {
	voidPayment: protectAction('pur_payments', async ({ request, locals, fetch }) => {
