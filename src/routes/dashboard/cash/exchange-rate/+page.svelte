...
<div class="flex flex-col gap-8 max-w-6xl mx-auto" in:fade>
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
    <div class="flex flex-col gap-2">
      ...
      <p class="text-text-muted text-lg">
        Actualiza la tasa oficial del Banco Central de Venezuela en todas tus sucursales.
      </p>
    </div>
    ...
    <div class="glass px-6 py-3 rounded-2xl border border-white/5 flex items-center gap-4">
      ...
      <div>
        <p class="text-[10px] uppercase tracking-widest font-black text-text-muted">Fecha Hoy</p>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1 flex flex-col gap-6">
      <div class="glass p-8 rounded-[2.5rem] border border-white/5 relative overflow-hidden group">
        <!-- Decoration -->
        <div class="absolute -top-12 -right-12 w-48 h-48 bg-brand-500/10 rounded-full blur-3xl transition-all group-hover:bg-brand-500/20"></div>
        <div class="absolute -bottom-8 -left-8 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl"></div>
        ...
        <div class="relative z-10 flex flex-col gap-6">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-1">
                ...
                <button 
                    ...
                    class="p-2 hover:bg-white/10 rounded-xl transition-colors text-text-muted {isEditing ? 'text-brand-400 bg-brand-500/10' : 'hover:text-brand-400'}"
          ...
          <div class="flex flex-col">
            <span class="text-[10px] uppercase tracking-widest font-black text-text-muted mb-1">Dólar Estadounidense (USD)</span>
      ...
      <!-- TIP CARD -->
      <div class="glass p-6 rounded-3xl border border-white/5 text-sm text-text-muted flex gap-4">
        <TrendingUp size={24} class="text-brand-400 shrink-0" />
        <p>
            Al presionar <strong>Sincronizar</strong>, el sistema actualizará automáticamente la tasa del dólar en el maestro de monedas de todas las sucursales autorizadas.
        </p>
      </div>
    </div>
    ...
    <div class="lg:col-span-2 flex flex-col gap-4">
        <div class="flex items-center justify-between px-2">
            ...
            <span class="text-xs text-text-muted font-medium bg-white/5 px-3 py-1 rounded-full border border-white/5">
        ...
        <div class="flex flex-col gap-3">
            {#each data.branchRates as branch}
                <div class="glass p-5 rounded-2xl border border-white/5 flex items-center justify-between group hover:bg-white/[0.02] transition-colors">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 bg-surface-base rounded-xl flex items-center justify-center border border-white/5 group-hover:border-brand-500/30 transition-colors">
                            <Store size={22} class="text-text-muted group-hover:text-brand-400 transition-colors" />
                        </div>
                        <div class="flex flex-col">
                            <h4 class="font-bold text-text-base">{branch.name}</h4>
                            <span class="text-xs text-text-muted font-medium uppercase tracking-tight">{branch.id}</span>
                        </div>
                    </div>

                    <div class="flex items-center gap-8">
                        <div class="flex flex-col items-end">
                            <span class="text-[10px] uppercase font-black text-text-muted tracking-widest mb-0.5">Tasa Actual</span>
                            <div class="flex items-center gap-2">
                                <span class="font-mono font-bold text-lg {branch.currentRate === data.bcvRate ? 'text-emerald-400' : 'text-white'}">
        ...
        {#if form?.results}
            <div class="mt-4 flex flex-col gap-2" in:slide>
                <h4 class="text-xs font-black uppercase tracking-widest text-text-muted px-2">Resultado de la última sincronización:</h4>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {#each form.results as res}
                        <div class="flex items-center gap-2 text-xs p-2 rounded-lg {res.success ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'} border border-white/5">
                            {#if res.success}
