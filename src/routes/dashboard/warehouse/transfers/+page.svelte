...
<script lang="ts">
  ...
  let filteredTransfers = $derived.by(() => {
    ...
    if (selectedBranch && selectedBranch !== "all") {
      if (activeTab === "incoming") {
        ...
        list = list.filter(
          (t: any) =>
            t.source_branch_id === selectedBranch ||
            t.target_branch_id === selectedBranch,
        );
      }
    } else {
      if (activeTab === "incoming") {
  ...
  let totalPages = $derived(
    Math.ceil(filteredTransfers.length / pageSize) || 1,
  );
  ...
  function exportTransferToExcel() {
    ...
    if (items.length === 0) {
      toast.error("El traslado no contiene artículos para exportar.");
      return;
    }
    ...
    // Membrete / Metadatos del Traslado
    csvContent += `TRASLADO DE INVENTARIO;${selectedTransfer.transfer_number || ""}\n`;
    csvContent += `Estatus;${selectedTransfer.status || ""}\n`;
    csvContent += `Sede Origen;${selectedTransfer.source_branch?.name || ""}\n`;
    csvContent += `Ajuste Salida Profit;${selectedTransfer.source_ajue_num || "N/A"}\n`;
    csvContent += `Sede Destino;${selectedTransfer.target_branch?.name || ""}\n`;
    csvContent += `Ajuste Entrada Profit;${selectedTransfer.target_ajue_num || "Pendiente"}\n`;
    csvContent += `Emitido Por;${selectedTransfer.created_by || ""}\n`;
    csvContent += `Fecha Emision;${dayjs(selectedTransfer.created_at).format("DD/MM/YYYY hh:mm A")}\n`;
    if (selectedTransfer.motivo) {
      const motivoClean = String(selectedTransfer.motivo)
        .replace(/"/g, '""')
        .replace(/\r?\n/g, " ");
      csvContent += `Motivo / Observacion;"${motivoClean}"\n`;
    }
    csvContent += "\n"; // Línea en blanco separadora

    // Encabezados de la tabla de artículos
    csvContent +=
      "Codigo;Descripcion;Almacen Origen;Almacen Destino;Cantidad;Unidad\n";
    ...
    for (const item of items) {
      ...
      const srcName = warehouseNames[item.co_alma_source]
        ...
        : `="${String(item.co_alma_source || "").trim()}"`;

      const tgtName = warehouseNames[item.co_alma_target]
        ? `"${warehouseNames[item.co_alma_target]} (${item.co_alma_target})"`
        : `="${String(item.co_alma_target || "").trim()}"`;

      const cantidad = (Number(item.total_art) || 0)
        .toFixed(2)
        .replace(".", ",");
      const unidad = `"${String(item.co_uni || item.unidad || "UND").trim()}"`;

      csvContent += `${co_art};${art_des};${srcName};${tgtName};${cantidad};${unidad}\n`;
    }
    ...
    const safeNum = String(
      selectedTransfer.transfer_number || "traslado",
    ).replace(/[^a-zA-Z0-9_-]/g, "_");
    const filename = `traslado_${safeNum}_${dayjs(selectedTransfer.created_at || new Date()).format("YYYYMMDD")}.csv`;
    ...
    toast.success("Detalle de traslado exportado a Excel.");
  }
  ...
  async function executeVoidEntry(e?: Event) {
    ...
    try {
      ...
      if (res.ok && resData.success) {
        toast.success(
          resData.message || "Ingreso de traslado anulado con éxito.",
        );
        ...
        toast.error(
          resData.message || "Error al anular el ingreso del traslado.",
        );
      }
    } catch (e: any) {
      toast.error(`Error de red: ${e.message}`);
  ...
  function getStatusBadge(status: string) {
    switch (status) {
      ...
      default:
        return {
          label: status,
          bg: "bg-white/10 border-white/20 text-text-muted",
          icon: Box,
        };
    }
  }
...
<div class="space-y-8" in:fade={{ duration: 150 }}>
  ...
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
    <div>
      <h1
        class="text-3xl font-black tracking-tight flex items-center gap-3 text-text-base"
      >
        <ArrowRightLeft size={36} class="text-brand-500" />
        {data.title}
      </h1>
      <p class="text-text-muted mt-2 text-base">
        Control de envío y recepción de inventario mediante Ajustes de Salida y
        Entrada entre sedes.
      </p>
    </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div
      class="glass p-6 rounded-3xl border border-white/5 shadow-xl relative overflow-hidden"
    >
      <div class="flex items-center justify-between">
        <div>
          <span
            class="text-xs font-bold uppercase tracking-widest text-text-muted"
            >En Tránsito / Pendientes</span
          >
        ...
        <div
          class="h-12 w-12 rounded-2xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400"
        >
          <Truck size={24} />
        </div>
      </div>
    ...
    >
      <div class="flex items-center justify-between">
        ...
        <div
          class="h-12 w-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400"
        >
          <CheckCircle size={24} />
        </div>
      </div>
    ...
    >
      <div class="flex items-center justify-between">
        <div>
          ...
          >
          <div class="text-3xl font-black text-brand-400 mt-2">
            {metrics.total}
          </div>
        </div>
        <div
  ...
  <!-- FILTROS Y CONTROLES -->
  <div
    class="glass p-6 rounded-3xl border border-white/5 shadow-2xl flex flex-col md:flex-row gap-4 items-center justify-between relative z-20"
  >
    <!-- Tabs Navegación -->
    <div
      class="flex items-center bg-white/5 border border-white/5 p-1.5 rounded-2xl w-full md:w-auto"
    >
      <button
        onclick={() => (activeTab = "incoming")}
        class="px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-2 {activeTab ===
        'incoming'
    ...
    <div class="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
      ...
      <div class="relative w-full sm:w-64">
        ...
        <Search
          size={16}
          class="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-muted opacity-50"
        />
      </div>
    </div>
  </div>
  ...
  {#if filteredTransfers && filteredTransfers.length > 0}
    <div
      class="glass rounded-[32px] border border-white/5 overflow-hidden shadow-2xl"
    >
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr
              class="border-b border-border-subtle bg-surface-soft/50 text-text-muted text-xs font-bold uppercase tracking-wider"
            >
          ...
          <tbody class="divide-y divide-border-subtle/40 text-sm font-semibold">
            {#each paginatedTransfers as item, idx}
              ...
              <tr class="hover:bg-surface-soft/40 transition-colors">
                <td
                  class="px-6 py-4 font-mono font-black text-brand-500 text-xs"
                  >#{itemIndex}</td
                >
                <td class="px-6 py-4 text-text-base"
                  >{item.source_branch?.name || "---"}</td
                >
                <td class="px-6 py-4 font-mono text-xs text-amber-500 font-bold"
                  >{item.source_ajue_num || "Pendiente"}</td
                >
                <td class="px-6 py-4 text-text-base"
                  >{item.target_branch?.name || "---"}</td
                >
                <td
                  class="px-6 py-4 font-mono text-xs text-emerald-500 font-bold"
                  >{item.target_ajue_num || "Sin Ingresar"}</td
                >
                <td class="px-6 py-4 text-text-muted text-xs font-mono"
                  >{dayjs(item.created_at).format("DD/MM/YYYY hh:mm A")}</td
                >
                <td class="px-6 py-4 text-center">
                  <span
                    class="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider border {badge.bg}"
                  >
                    <badge.icon size={12} />
                    {badge.label}
                  </span>
      ...
      {#if totalPages > 1}
        <div
          class="px-8 py-5 bg-surface-soft/40 border-t border-border-subtle flex items-center justify-between"
        >
          <p
            class="text-[10px] font-black uppercase tracking-[0.2em] text-text-muted"
          >
            Página <span class="text-text-base font-bold">{currentPage}</span>
            de
            <span class="text-text-base font-bold">{totalPages}</span> (Total: {filteredTransfers.length}
            traslados)
          </p>
  ...
  {:else}
    <div
      class="glass p-12 rounded-[40px] border border-white/5 flex flex-col items-center justify-center text-center space-y-4 min-h-[350px]"
    >
      <div
        class="h-20 w-20 rounded-3xl bg-white/5 flex items-center justify-center text-text-muted/20"
      >
        <ArrowRightLeft size={48} />
      </div>
      <h2 class="text-2xl font-bold text-text-muted">
        Sin traslados registrados
      </h2>
      <p class="text-text-muted/60 max-w-sm">
        No se encontraron movimientos con los filtros aplicados. Puedes
        registrar un nuevo envío entre sedes.
      </p>
    </div>
  {/if}
</div>
  ...
  >
    ...
    >
      <div class="p-6 md:p-8 space-y-6">
        ...
        <div class="text-center space-y-3">
          ...
          <div class="space-y-1">
            ...
            <p class="text-text-muted text-xs leading-relaxed max-w-md mx-auto">
              Previsualización de membrete, observaciones y artículos en
              tránsito entre sedes.
            </p>
          </div>
        </div>
        ...
        <div
          class="p-4.5 rounded-2xl bg-surface-soft border border-border-subtle text-left space-y-3 text-xs"
        >
          <div
            class="flex items-center justify-between border-b border-border-subtle pb-2.5"
          >
            <div>
              <span
                class="text-[10px] font-black uppercase tracking-wider text-text-muted block"
                >Traslado Correlativo</span
              >
              <span class="font-black text-brand-500 text-sm font-mono"
                >{selectedTransfer.transfer_number}</span
              >
            </div>
            <span
              class="px-2.5 py-1 rounded-lg border text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 {badge.bg}"
            >
              <badge.icon size={12} />
              {badge.label}
            </span>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-0.5">
              <span
                class="text-[10px] text-text-muted font-black uppercase tracking-wider block"
                >Sede Origen</span
              >
              <span class="font-bold text-text-base text-xs block"
                >{selectedTransfer.source_branch?.name || "---"}</span
              >
              {#if selectedTransfer.source_ajue_num}
                <span
                  class="text-[10px] text-amber-500 font-mono font-bold block"
                  >Ajuste Salida Profit: {selectedTransfer.source_ajue_num}</span
                >
              {/if}
            </div>
            ...
            <div class="space-y-0.5">
              ...
              >
              <span class="font-bold text-text-base text-xs block"
                >{selectedTransfer.target_branch?.name || "---"}</span
              >
              {#if selectedTransfer.target_ajue_num}
                <span
                  class="text-[10px] text-emerald-500 font-mono font-bold block"
                  >Ajuste Entrada Profit: {selectedTransfer.target_ajue_num}</span
                >
              {:else}
                <span
                  class="text-[10px] text-amber-500 font-mono font-bold block"
                  >Ajuste Entrada Pendiente</span
                >
              {/if}
            </div>
          </div>
          ...
          {#if selectedTransfer.created_by || selectedTransfer.created_at || selectedTransfer.motivo}
            <div
              class="pt-2 border-t border-border-subtle space-y-1 text-[11px] text-text-muted"
            >
              {#if selectedTransfer.motivo}
                <div>
                  <strong class="text-text-base font-semibold"
                    >Observación / Motivo:</strong
                  >
                  {selectedTransfer.motivo}
                </div>
              {/if}
              <div
                class="flex items-center justify-between text-[10px] text-text-muted pt-0.5"
              >
                <span
                  >Emitido por: <strong class="text-text-base font-bold"
                    >{selectedTransfer.created_by || "---"}</strong
                  ></span
                >
                <span
                  >Fecha: <strong class="text-text-base font-bold"
                    >{dayjs(selectedTransfer.created_at).format(
                      "DD/MM/YYYY hh:mm A",
                    )}</strong
                  ></span
                >
        ...
        <div class="space-y-2 text-left">
          <div class="flex items-center justify-between">
            ...
            <!-- <button
              ...
              class="px-3 py-1.5 rounded-xl bg-brand-600/20 hover:bg-brand-600/30 text-brand-400 border border-brand-500/30 font-bold text-[11px] transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
              title="Exportar artículos del traslado a Excel"
            >
          ...
          <div
            class="border border-border-subtle rounded-2xl overflow-hidden max-h-56 overflow-y-auto custom-scrollbar bg-surface-base/50"
          >
            <table class="w-full text-left text-xs">
              ...
              >
                {#each selectedTransfer.items || [] as item}
                  {@const srcName = warehouseNames[item.co_alma_source]
                    ? `${warehouseNames[item.co_alma_source]} (${item.co_alma_source})`
                    : `Almacén ${item.co_alma_source}`}
                  {@const tgtName = warehouseNames[item.co_alma_target]
                    ? `${warehouseNames[item.co_alma_target]} (${item.co_alma_target})`
                    : `Almacén ${item.co_alma_target}`}
                  <tr class="hover:bg-surface-soft/50">
                    ...
                    >
                    <td
                      class="p-3 text-center font-mono text-text-muted text-[11px]"
                    >
                    ...
                    <td
                      class="p-3 text-right font-mono font-black text-emerald-500 text-xs whitespace-nowrap"
                    >
                      {Number(item.total_art).toFixed(2)}
                      <span
                        class="text-[10px] text-emerald-600 font-bold ml-1 uppercase"
                        >{item.co_uni || item.unidad || "UND"}</span
                      >
                    </td>
                  </tr>
                {/each}
        ...
        <div
          class="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-border-subtle/40"
        >
          <button
            type="button"
            onclick={exportTransferToExcel}
            class="w-full sm:w-auto h-12 px-6 rounded-2xl font-bold bg-brand-600 hover:bg-brand-500 text-white shadow-lg shadow-brand-600/20 hover:shadow-brand-500/30 transition-all flex items-center justify-center gap-2 text-xs cursor-pointer"
            title="Exportar artículos del traslado a Excel"
          >
          ...
          <div
            class="flex flex-col sm:flex-row items-center justify-end gap-3 w-full sm:w-auto"
          >
            ...
            {#if data.canVoid && selectedTransfer.status === "ACEPTADO" && (selectedBranch === "all" ? selectedTransfer.target_branch_id === data.userBranchId : selectedTransfer.target_branch_id === selectedBranch)}
              <button
                ...
                onclick={() => {
                  detailModalOpen = false;
                  promptVoidEntry(selectedTransfer);
                }}
                class="w-full sm:w-auto h-12 px-6 rounded-2xl font-bold bg-amber-500/20 hover:bg-amber-500/30 text-amber-400 border border-amber-500/30 transition-all flex items-center justify-center gap-2 text-xs cursor-pointer"
              >
                <Ban size={16} />
            ...
            {#if selectedTransfer.status === "TRANSITO" && (selectedBranch === "all" || selectedTransfer.target_branch_id === selectedBranch)}
              <button
                ...
                onclick={() => {
                  detailModalOpen = false;
                  promptAcceptTransfer(selectedTransfer);
                }}
...
{#if showAcceptModal && transferToAccept}
  ...
  >
    <div
      ...
      onkeydown={(e) =>
        e.key === "Escape" && !isAccepting && (showAcceptModal = false)}
      role="button"
      tabindex="-1"
    ></div>
      ...
      <div class="p-6 md:p-8 space-y-6">
        ...
        <div class="text-center space-y-3">
          ...
          <div class="space-y-1">
            ...
            <p class="text-text-muted text-xs leading-relaxed max-w-md mx-auto">
              Revise el detalle de artículos y confirme con su contraseña para
              ingresar la mercancía al inventario de esta sede.
            </p>
          </div>
        </div>
        ...
        >
          ...
          >
            <div>
              ...
              >
              <span class="font-black text-brand-500 text-sm font-mono"
                >{transferToAccept.transfer_number}</span
              >
            </div>
            <span
              class="px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-500 border border-amber-500/20 text-[10px] font-black uppercase tracking-wider"
            >
              Pendiente
            </span>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-0.5">
              ...
              >
              <span class="font-bold text-text-base text-xs block"
                >{transferToAccept.source_branch?.name || "---"}</span
              >
              {#if transferToAccept.source_ajue_num}
                <span
                  class="text-[10px] text-amber-500 font-mono font-bold block"
                  >Ajuste Salida Profit: {transferToAccept.source_ajue_num}</span
                >
              {/if}
            </div>
            ...
            <div class="space-y-0.5">
              ...
              >
              <span class="font-bold text-text-base text-xs block"
                >{transferToAccept.target_branch?.name || "---"}</span
              >
              <span
                class="text-[10px] text-emerald-500 font-mono font-bold block"
                >Ajuste Entrada Pendiente</span
          ...
          {#if transferToAccept.created_by || transferToAccept.created_at || transferToAccept.motivo}
            ...
            >
              ...
              >
                <span
                  >Emitido por: <strong class="text-text-base font-bold"
                    >{transferToAccept.created_by || "---"}</strong
                  ></span
                >
                <span
                  >Fecha: <strong class="text-text-base font-bold"
                    >{dayjs(transferToAccept.created_at).format(
                      "DD/MM/YYYY hh:mm A",
                    )}</strong
                  ></span
                >
...
{#if showVoidEntryModal && transferToVoidEntry}
  ...
  >
    ...
    >
      <div
        class="w-16 h-16 rounded-2xl bg-amber-500/10 text-amber-500 flex items-center justify-center mx-auto border border-amber-500/20"
      >
        <Ban size={32} />
      </div>

      <div class="space-y-2">
        <h3 class="text-xl font-black text-text-base">
          Anular Ingreso de Traslado
        </h3>
        <p class="text-xs text-text-muted">
          Esta acción anulará el ajuste de entrada en la sede destino y
          revertirá el estado del traslado <span
            class="font-mono font-bold text-amber-500"
            >{transferToVoidEntry.transfer_number}</span
          >
          a <span class="font-bold text-amber-400">PENDIENTE</span> en la nube.
        </p>
      </div>

      <!-- RESUMEN DEL TRASLADO -->
      <div
        class="bg-surface-soft/60 rounded-2xl p-4 border border-border-subtle text-left space-y-2 font-mono text-xs"
      >
        ...
        <div class="flex justify-between">
          ...
          <span class="font-bold text-emerald-500"
            >{transferToVoidEntry.target_ajue_num || "---"}</span
          >
        </div>
        <div class="flex justify-between">
          <span class="text-text-muted">Sede Destino:</span>
          <span class="font-bold text-text-base"
            >{transferToVoidEntry.target_branch?.name || "---"}</span
          >
        </div>
        <div class="flex justify-between">
          <span class="text-text-muted">Total Renglones:</span>
          <span class="font-bold text-amber-500"
            >{transferToVoidEntry.items?.length || 0} artículos</span
          >
        </div>
      </div>
            ...
            >
              {#each transferToVoidEntry.items || [] as item}
                ...
                <tr class="hover:bg-surface-soft/50">
                  ...
                  >
                  <td
                    class="p-3 text-right font-mono font-black text-amber-500 text-xs whitespace-nowrap"
                  >
                    {Number(item.total_art).toFixed(2)}
                    <span
                      class="text-[10px] text-amber-600 font-bold ml-1 uppercase"
