// src/routes/dashboard/reports/article-sales/+page.server.ts
import { protectLoad } from '$lib/server/permissions';
import { AgentClient } from '$lib/server/agent';
import type { PageServerLoad } from './$types';

export const load: PageServerLoad = protectLoad('reports_article_sales', async ({ url, locals, fetch }) => {
    ...
    try {
        ...
        const agentClient = new AgentClient({
            ...
            agent_token: selectedBranch.agent_token
        }, profile, fetch);
        ...
        console.error("[ARTICLE SALES REPORT SERVER LOAD ERROR]:", err);
        return { 
            report: { data: [] }, 
            branches: [], 
