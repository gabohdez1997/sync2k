...
export const POST: RequestHandler = async ({ request, locals, fetch }) => {
	try {
		...
		if (!hasPermission(profile, 'reports_payables', 'update')) {
			return json({ success: false, message: 'No tienes permiso para editar descuentos en cuentas por pagar.' }, { status: 403 });
		}

		const body = await request.json();
		const { co_tipo_doc, nro_doc, campo1, campo2, campo3, sede_id } = body;
		...
		const response = await agentClient.request<any>('/reportes/cxp/descuentos', {
			method: 'POST',
			body: JSON.stringify({ co_tipo_doc, nro_doc, campo1, campo2, campo3, sede_id })
		});
		...
		console.error('[API DESCUENTOS PROXY] Error:', e.message);
