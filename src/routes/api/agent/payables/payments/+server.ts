...
export const GET: RequestHandler = async ({ url, locals, fetch }) => {
	try {
		...
		const resData = await agentClient.request<any>(`/pagos?${params.toString()}`);
		...
		console.error('[API PAYABLES PAYMENTS GET] Error:', e.message);
...
export const POST: RequestHandler = async ({ request, url, locals, fetch }) => {
	try {
		...
		const resData = await agentClient.request<any>(`/pagos`, {
			method: 'POST',
		...
		if (resData && (resData.success || resData.success !== false)) {
			try {
				...
				await supabaseAdmin.from('audit_log').insert({
					...
					metadata: {
						message: `Pago ${docNum} creado con éxito`,
						doc_num: docNum,
						prov_code: body.co_prov,
						total_monto: body.monto
					}
				});
			} catch (auditError) {
				console.error('Error al guardar log de auditoría de pago:', auditError);
			}
		}

		return json(resData);
	} catch (e: any) {
		console.error('[API PAYABLES PAYMENTS POST] Error:', e.message);
		return json({ error: e.message }, { status: 500 });
	}
};
